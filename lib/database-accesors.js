...
class Query {
  constructor(app, path = '/', config) {
    ...
    */
    ...
    this._getOnceCallback = (resolve, callback) => (snapshot) => {
    ...
    this.once = (eventType, callback, failureCallbackOrContext, context) =>
      new Promise((resolve, reject) => {
        try {
          const onceCallback = this._getOnceCallback(resolve, callback);
    ...
    this.off = (eventType, callback/* , context */) => {
      ...
      const isFirebaseEventHandler = callback[LOCATION] &&
        callback[LOCATION][eventType] &&
        callback[LOCATION][eventType][this._cleanPath];

      if (isFirebaseEventHandler) {
...
class Reference extends Query {
  constructor(app, path = '/', config) {
    ...
    this.push = (value, onComplete) => {
      ...
      const childPath = `${this._cleanPath}/${firebaseId}`;

      if (value === undefined) {
        // eslint-disable-next-line no-use-before-define
        return new ThenableReference(app, childPath, config);
      }
    ...
    this.update = (values, onComplete) => {
      ...
      const nextTree = Object.keys(values).reduce((acc, key) => {
        const location = [...this._pathArray, ...getPaths(key)];
        return setValue(acc, values[key], location);
      }, this._database._getData());
...
class ThenableReference extends Reference {
  constructor(app, path = '/', config) {
    super(app, path, config);
    if (this.constructor === ThenableReference) {
      this.ref = new Reference(app, path, config);
    }
    this._dataSnapshot = Promise.resolve(this._getDataSnapshot());

    this.then = callback => this._dataSnapshot.then(callback);
