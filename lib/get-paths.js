...
module.exports = (path) => {
  ...
  if (!isValidFirebaseKey.test(path)) {
    throw new Error(`Invalid firebase key: ${path}`);
  }
  ...
  // empty strings anywhere else indicate they key had extra slashes and is incorrect
  if (paths.includes('')) {
    throw new Error(`Invalid firebase key: ${path}`);
  }
