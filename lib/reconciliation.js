...
module.exports = (prev, next) => {
  ...
  const lacksPropertyKeys = Object.keys(updates).length === 0;
  const lacksUpdateKeys = Object.getOwnPropertySymbols(updates).length === 0;
  if (lacksPropertyKeys && lacksUpdateKeys) {
    return null;
  }
