...
const recursiveReconciliation = (updates, location, prev, next) => {
  ...
  if (prevIsObject && !nextIsObject) {
    prev.forEach((child, childKey) => {
      recursiveReconciliation(updates, [...location, childKey], child, null);
    });
    return;
  }
  ...
  if (!prevIsObject && nextIsObject) {
    next.forEach((child, childKey) => {
      recursiveReconciliation(updates, [...location, childKey], null, child);
    });
    return;
  }
  ...
  Object.keys(keys).forEach((key) => {
    ...
    recursiveReconciliation(updates, [...location, key], prevChild, nextChild);
  });
};
...
module.exports = (prev, next) => {
  const updates = [];
  const location = [];
  recursiveReconciliation(updates, location, prev, next);
  return updates;
};
