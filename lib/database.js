...
class Database {
  constructor(app) {
    ...
    this._pushKeys = [];
    this.getPushKeys = () => this._pushKeys;

    // INTERNALS
    // fetches data as an ImmutableJS object
    this._getData = () => this._data;

    // generates a firebase push token
    this._generateFirebaseToken = () => {
      const token = generateFirebaseToken();
      this._pushKeys.push(token);
      return token;
    };
    ...
    this._registerCallback = (eventType, path, location, index, callback, config) => {
      if (!this._callbacks[eventType][path]) {
        this._callbacks[eventType][path] = {};
      }
      this._callbacks[eventType][path][index] = {
        eventType,
        path,
        location,
        index,
      ...
      const nextNested = getValue(this._data, location);
    ...
    this._unregisterCallback = (eventType, location, index) => {
      ...
      if (Object.keys(this._callbacks[eventType][location]).length === 0) {
        delete this._callbacks[eventType][location];
      }
    };
    ...
    this._childInfoGenerator = function* childInfoGenerator(
      ...
      const getChildSnapshot = (key) => {
        if (!dataSnapshots[key]) {
          ...
          dataSnapshots[key] = new DataSnapshot(refChild, getValue(data, [key]));
        }
