...
class Database {
  constructor(app) {
    ...
    this._registerCallback = (eventType, location, index, callback) => {
      if (eventType === 'value') {
        this._registerCallbackForEvent('child_added', location, index, callback);
        this._registerCallbackForEvent('child_changed', location, index, callback);
        this._registerCallbackForEvent('child_removed', location, index, callback);
        this._fireCallbacks(eventType, location, index);
      } else {
        this._registerCallbackForEvent(eventType, location, index, callback);
      }
    };
    ...
    this._unregisterCallback = (eventType, location, index) => {
      ...
      if (eventType === 'value') {
        this._unregisterCallbackForEvent('child_added', location, index);
        this._unregisterCallbackForEvent('child_changed', location, index);
        this._unregisterCallbackForEvent('child_removed', location, index);
      } else {
        this._unregisterCallbackForEvent(eventType, location, index);
      }
    };
